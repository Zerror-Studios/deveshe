...
const ProductPage = () => {
		...
		try {
			if (id) {
				...
				if (storedProductData) {
					...
					localStorage.setItem(`product_${id}_expires`, expirationTime.toString());
				} else {
	...
	const handleEnablebtn = () => {
		if (product) {
			if (product.colorVar) {
				if (Object.keys(selectedVarients).length + 1 == product.variants.length + 1) {
	...
	function isObjectMatch(obj, criteria) {
		return Object.keys(criteria).every((key) => obj.some((prop) => prop[key] === criteria[key]));
	}
	...
	useEffect(() => {
		if (window.innerWidth >= 1000) {
			...
			if (container) {
				...
				console.error('Element with className "ProductDets_Big_img_wrap" not found.');
			}
	...
	useEffect(() => {
		const handleResize = () => {
			if (window.innerWidth >= 0) {
				const ReactModalPortalImageCntr = document.querySelector(".ReactModalPortal_img_cntr");
				const cursor = document.querySelector(".ReactModalPortal_img_cntr_cursor");
				const allBigImgCntr = document.querySelectorAll(".ProductDets_Big_card_img-main_cntr img");
										...
										<div className="ProductDets_img_slider_cntr">
											<div className="ProductDets_img_slider_cntr_sticky">
												{images &&
													images.map((items, i) => {
														return (
															<button key={i} className="ProductDets_img_btn ProductDets_img_align">
													...
													<button key={items.id} className="ProductDets_Big_img_cntr">
														<div className="shop_card_img_bgcover">
															<div className="ProductDets_Big_card_img-main_cntr">
																<Image width={1000} height={1000} src={`${items}`} alt={`images`} />
															</div>
														</div>
													</button>
												);
                      ...
                      zakary
                        ...
                        Description
													...
													<div className="ProductDets_text-container_prdt_Desc">
														<div>{desc && <div dangerouslySetInnerHTML={{ __html: desc }} />}</div>
														<div>
															Fits large to size, we suggest taking one size smaller than usual.
														</div>
													</div>
												</div>
													...
													<div className="ProductDets_collection-wrap">
														<fieldset className="ProfuctDets_fieldset">
															{product &&
																...
																product.productOptions
																	.filter((option) => option.showInProductPageAs === "Color")
																	.map((colorOption) =>
												...
												<div className="ProductDets_size_Mainwrap">
													{product &&
														...
														product.productOptions
															...
															.map((productOption, i) => (
																<div className="ProductDets_size_wrap" key={`productOption-${i}`}>
																			...
																			{productOption.choices &&
																				productOption.choices.map((choice, j) => (
																					<div
																						key={`choice-${j}`}
																						onClick={() => {
																							handleVariants(productOption.optionName, choice.name);
													...
													{btnLoading ? (
														...
														<>
															{!enableAddToCart ? (
																...
																<span className="ProductDets_ntfy_btn_slect_size">Add to Bag</span>
															)}
